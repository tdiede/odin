...
(defn new-note-form [account]
  (let [form        (subscribe [:accounts.entry.create-note/form-data (:id account)])
        can-create  (subscribe [:accounts.entry/can-create-note? (:id account)])
        is-creating (subscribe [:ui/loading? :accounts.entry/create-note!])
        is-showing  (subscribe [:accounts.entry.create-note/showing?])]
    (fn [account]
      (if @is-showing
        [ant/card
         [note-form
          {:subject     (:subject @form)
           ...
           :on-cancel   #(do (dispatch [:accounts.entry.create-note/toggle])
                             (dispatch [:accounts.entry.create-note/clear (:id account)]))
           :on-change   (fn [k v] (dispatch [:accounts.entry.create-note/update (:id account) k v]))
           :is-loading  @is-creating
           :disabled    (not @can-create)
           :on-submit   #(dispatch [:accounts.entry/create-note! (:id account) @form])
           :button-text "Create"}]]
        [ant/button
         {:type     :primary
          :size     :large
...
(defn- edit-note-form
  ...
  ([note is-comment]
   (let [form      (r/atom (select-keys note [:subject :content]))
         is-saving (subscribe [:ui/loading? :accounts.entry/update-note!])]
     (fn [note is-comment]
       [note-form
        {:subject    (:subject @form)
         :content    (:content @form)
         ...
         :on-cancel  #(dispatch [:accounts.entry.note/toggle-editing (:id note)])
         :on-submit  #(dispatch [:accounts.entry/update-note! (:id note) @form])}]))))
...
(defn- note-actions
  ...
  ([{:keys [id subject content created updated author] :as note} is-comment]
   (let [is-deleting   (subscribe [:ui/loading? :accounts.entry.note/delete])
         is-commenting (subscribe [:accounts.entry.note/comment-form-shown? id])
         account       (subscribe [:user])
         is-author     (= (:id author) (:id @account))]
     [:div.actions
      (when-not is-comment
        [note-action
         {:on-click #(dispatch [:accounts.entry.note/toggle-comment-form id])}
         [:span
          [ant/icon {:type (if @is-commenting "up" "down")}]
          " Comment"]])
      (when is-author
        [note-action
         {:on-click #(dispatch [:accounts.entry.note/toggle-editing id])}
         "Edit"])
      (when is-author
        [note-action
         {:class    "text-red"
          :on-click (fn []
                      (ant/modal-confirm
                       {:title   "Are you sure?"
                        ...
                        :on-ok   #(dispatch [:accounts.entry.note/delete! id])}))}
         "Delete"])])))
...
(defn- comment-form [note]
  (let [text          (subscribe [:accounts.entry.note/comment-text (:id note)])
        is-commenting (subscribe [:ui/loading? :accounts.entry.note/add-comment!])]
    [:article.media
     ...
     [:div.media-content
      [:form
       {:on-submit
        #(do
           (.preventDefault %)
           (dispatch [:accounts.entry.note/add-comment! (:id note) @text]))}
       [ant/form-item
        [ant/input
         {:type          :textarea
          :cols          3
          ...
          :default-value @text
          :on-change     #(dispatch [:accounts.entry.note.comment/update
...
(defn- note-body
  ...
  ([{:keys [id subject content comments] :as note} is-comment]
   (let [is-commenting (subscribe [:accounts.entry.note/comment-form-shown? id])]
     [:article.media
      (when is-comment
        [:figure.media-left [ant/icon {:type "message"}]])
      [:div.media-content
...
(defn- note-content
  ...
  ([note is-comment]
   (let [is-editing (subscribe [:accounts.entry.note/editing (:id note)])]
     (if-not @is-editing
       ;; not editing
